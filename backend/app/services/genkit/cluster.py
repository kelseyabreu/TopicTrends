...
try:
    if AI_PROVIDER == "ollama":
        ...
        logger.info(f"Initialized Genkit with Ollama provider")
    elif AI_PROVIDER == "googleai":
        ai = Genkit(
            plugins=[GoogleAI(api_key=settings.GOOGLE_API_KEY)],
        )
        logger.info(f"Initialized Genkit with Google AI provider")
    else:
        raise ValueError(f"Unsupported AI provider: {AI_PROVIDER}")
except Exception as e:
    logger.critical(f"Failed to initialize Genkit with {AI_PROVIDER}: {e}", exc_info=True)
    raise SystemExit(f"Genkit initialization failed: {e}")
...
async def embed_ideas_batched(ideas: list) -> list:
    ...
    try:
        ...
        documents_to_embed = [Document.from_text(idea["text"]) for idea in ideas_needing_embedding]
        ...
        else:  # Google AI or other providers
            embedding_response = await ai.embed(
                embedder=EMBEDDER_MODEL,
                documents=documents_to_embed,
            )
            
        if not embedding_response.embeddings or len(embedding_response.embeddings) != len(ideas_needing_embedding):
            logger.error(f"Embedding response length mismatch. Expected {len(ideas_needing_embedding)}, got {len(embedding_response.embeddings) if embedding_response.embeddings else 0}")
        ...
        if update_tasks:
            ...
            successful_updates = sum(1 for r in results if not isinstance(r, Exception) and r.modified_count > 0)
            failed_updates = len(results) - successful_updates
            logger.info(f"Updated embeddings in DB: {successful_updates} successful, {failed_updates} failed")
            
            # Log errors if any
            for i, res in enumerate(results):
                if isinstance(res, Exception):
                    logger.error(f"Failed DB update for idea {idea_ids_to_update[i]}: {res}")
...
def _map_ideas_for_output(ideas_list: list) -> list:
    ...
    for idea in ideas_list:
        ...
        mapped_idea = {
            ...
            "timestamp": idea["timestamp"].isoformat() if isinstance(idea.get("timestamp"), datetime) else str(idea.get("timestamp", "")),
...
def find_nearest_ideas(input_embedding: list[float], ideas: list, top_n: int = 3) -> list[Idea]:
    """
    ...
    try:
        ...
        valid_ideas = [idea for idea in ideas if isinstance(idea.get("embedding"), list) and len(idea["embedding"]) > 0]
        ...
        for idea in valid_ideas:
            try:
                ...
                logger.warning(f"Error calculating distance for idea {idea.get('_id', 'unknown')}: {e}")
...
async def perform_clustering(embedded_ideas: list, distance_threshold: float) -> np.ndarray | None:
    """
    ...
    try:
        # Filter ideas with valid embeddings
        ideas_to_cluster = [idea for idea in embedded_ideas 
        ...
        if embeddings.ndim != 2:
            logger.error(f"Embeddings array has unexpected dimensions: {embeddings.ndim}")
            return None
...
def calculate_distance_threshold(idea_count: int) -> float:
    ...
    if idea_count < 50:
        ...
        logger.info(f"Using small distance threshold ({threshold}) for {idea_count} ideas")
    elif idea_count < 200:
        threshold = settings.DISTANCE_THRESHOLD_MEDIUM
        logger.info(f"Using medium distance threshold ({threshold}) for {idea_count} ideas")
    else:
        threshold = settings.DISTANCE_THRESHOLD_LARGE
        logger.info(f"Using large distance threshold ({threshold}) for {idea_count} ideas")
    return threshold
...
async def group_ideas_by_topic(labels: np.ndarray, ideas: list) -> dict:
    ...
    try:
        ...
        ideas_clustered = [idea for idea in ideas 
                          if isinstance(idea.get("embedding"), list) and len(idea["embedding"]) > 0]
        
        if len(labels) != len(ideas_clustered):
            logger.error(f"Label count ({len(labels)}) does not match clustered idea count ({len(ideas_clustered)})")
            return {}
    ...
    try:
        ...
        for label, topic_ideas_data in topics_temp_data.items():
            ...
            try:
                ...
                title = topic_main_idea.get('representative_text', representative_idea_data['text'][:100])
            except Exception as e:
            ...
            for idea in topic_ideas_data:
                idea_output = {
                    ...
                    "user_id": idea.get("user_id"),
                    "verified": idea.get("verified", False),
                    "timestamp": idea["timestamp"].isoformat() if isinstance(idea["timestamp"], datetime) else str(idea["timestamp"]),
                    "on_topic": idea.get("on_topic", 0),
                    "keywords": idea.get("keywords", []),
                    "intent": idea.get("intent"),
                    "sentiment": idea.get("sentiment"),
        ...
        if all_tasks:
            ...
            logger.info(f"Completed {success_count}/{len(all_tasks)} database operations")
...
async def generate_topic_results(topic_id: str, topics_temp_data: dict) -> list:
    ...
    try:
        ...
        logger.info(f"Generated {len(topic_results)} subtopic results for topic {topic_id}")
        return topic_results
    ...
    try:
        ...
        # Path 2: Full clustering for a discussion
            ...
            if persist_results:
                ...
                for label, ideas_data in topics_temp_data.items():
                    ...
                    cluster_embeddings = np.array([idea["embedding"] for idea in ideas_data])
                    if cluster_embeddings.shape[0] > 1:
                        centroid = np.mean(cluster_embeddings, axis=0)
                        distances = np.linalg.norm(cluster_embeddings - centroid, axis=1)
                        closest_idx = np.argmin(distances)
                    ...
                    temp_results.append({
                        ...
                        "representative_idea_id": representative_idea_data["id"],
                        "representative_text": representative_idea_data.get("text", "")[:100],
        ...
        if persist_results and discussion_id:
            try:
                await sio.emit(
                    "processing_error",
                    {"discussion_id": discussion_id, "error": "Clustering process failed"},
                    room=discussion_id
                )
            except Exception as emit_error:
                logger.error(f"Failed to emit error notification: {emit_error}")
...
async def recalculate_topic_metadata(topic_id: str) -> bool:
    ...
    try:
        ...
        avg_on_topic = sum(on_topic_scores) / len(on_topic_scores) if on_topic_scores else None
...
async def recommend_topics_for_user(user_id: str, discussion_id: str, limit: int = 5) -> list:
    """
    ...
    try:
        ...
        if not user_ideas:
            logger.info(f"No ideas found for user {user_id} in discussion {discussion_id}")
            return []
        ...
        logger.info(f"Generated {len(recommendations)} topic recommendations for user {user_id}")
...
async def analyze_topic_evolution(discussion_id: str, time_window_hours: int = 24) -> dict:
    """
    ...
    try:
        ...
        results = {
            ...
            "recent_ideas": sum(1 for idea in ideas if idea.get("timestamp") > cutoff_time),
            "topic_activity": [],
            "trending_topics": [],
            "declining_topics": []
        }
        ...
        logger.info(f"Completed topic evolution analysis for discussion {discussion_id}")
        return results
...
async def cluster_discussion_topics_background(discussion_id: str):
    ...
    try:
        ...
        # Run clustering
        await cluster_ideas_into_topics(discussion_id=discussion_id, persist_results=True)
...
async def drill_down_topic(topic_id: str) -> list:
    ...
    try:
        ...
        subtopics = await cluster_ideas_into_topics(topic_id=topic_id, persist_results=False)
        
        logger.info(f"Completed drill-down for topic {topic_id}, found {len(subtopics)} subtopics")
        return subtopics
