...
class MainIdea(BaseModel):
    """An overall main idea for a cluster of ideas."""
...
@ai.flow()
async def group_name_suggestion_flow(group: list) -> MainIdea:
    # Create a formatted list of idea texts
    idea_texts = "\n".join([f"- {idea['text']}" for idea in group])
    response = await ai.generate(
        model='googleai/gemini-2.0-flash',
        prompt=f'Whats the general idea for this group of ideas:\n{idea_texts}',
        output_schema=MainIdea
    )
    logging.info(f"group_name_suggestion_flow!!: {response.output}")
